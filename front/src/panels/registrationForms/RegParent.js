...
import {Group,
        ...
        Select,
        NativeSelect,
        ChipsSelect} from '@vkontakte/vkui'
import { checkPropTypes } from 'prop-types'
import _ from 'lodash'

class RegParent extends React.Component {
  constructor() {
  ...
  formChecking() {
        ...
        if (formElement === 'emailAddress') {
          if (state.formElements.emailAddress.content === '') {
            ...
            if (/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/.test(state.formElements.emailAddress.content)) {
              ...
              state.formElements.emailAddress.isCorrect = false
              state.isFormCorrect = false
              state.formElements[formElement] = {...(state.formElements[formElement]), errorName: 'Введен невалидный адрес электронной почты'}
            }
          }
        }
          ...
          if (state.formElements.phoneNumber.content === '') {
            ...
            if (/^((8|\+7)[\- ]?)?(\(?\d{3}\)?[\- ]?)?[\d\- ]{7,10}$/.test(state.formElements.phoneNumber.content)) {
              ...
              state.formElements.phoneNumber.isCorrect = false
              state.isFormCorrect = false
              state.formElements[formElement] = {...(state.formElements[formElement]), errorName: 'Введен невалидный номер телефона'}
            }
          }
        }
        else if (formElement === 'patronym' || formElement === 'city') {
    ...
    /*
    setTimeout(() => {
      this.sendData(this.state.formElements)
    }, 1000)
    */
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.state.isReadyToSend) {
      ...
      fetch(this.state.server + "register/parent", {
        method: "POST",
        headers: {
